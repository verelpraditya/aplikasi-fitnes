...
$pdf->Cell(0,10,'LAPORAN DATA ABSENSI IJIN',0,0,'C');
$pdf->ln(5);
$pdf->setFont('Arial','',11); 
...
$sql = "SELECT i.*, i.nik AS nik_i, k.*, j.* FROM ijin i LEFT OUTER JOIN karyawan k ON i.nik=k.nik LEFT OUTER JOIN jabatan j ON k.kd_jabatan=j.kd_jabatan WHERE ((MONTH(tgl_mulai)='$bulan' OR MONTH(tgl_selesai)='$bulan') AND (YEAR(tgl_mulai)='$tahun' OR YEAR(tgl_selesai)='$tahun')) ORDER BY i.nik";

$n=1;  
$query = $myDb->select($sql);
while($result=$myDb->getRow($query)) {
	$pdf->Cell(7,5,$n, 1,0,'C');  
	$pdf->Cell(17,5,$result['nik_i'],1,0,'C');  
